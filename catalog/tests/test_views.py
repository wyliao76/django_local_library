...
class AuthorCreateTest(TestCase):
    ...
    def test_date_of_death_format(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
        correct_date_format = datetime.date(2018, 1, 5)
        ...
        self.assertEqual(response_date, correct_date_format)

    def test_redirects_to_all_authors_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author_create'), {'first_name': 'Daniel', 'last_name': 'Berg'})
        self.assertEqual(response.status_code, 302)
